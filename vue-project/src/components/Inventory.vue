...
<template>
  <div>
    ...
    <table>
      ...
      <tbody>
        <tr v-for="item in inventory" :key="item.id">
          <td>{{ item.id }}</td>
          <td v-if="editingItem === item.id"><input v-model="updatedItem.reorder_level" type="number" /></td>
          <td v-else>{{ item.reorder_level }}</td>

          <td v-if="editingItem === item.id"><input v-model="updatedItem.stock_quantity" type="number" /></td>
          <td v-else>{{ item.stock_quantity }}</td>

          <td v-if="editingItem === item.id"><input v-model="updatedItem.restock_date" type="datetime-local" /></td>
          <td v-else>{{ item.restock_date ? new Date(item.restock_date).toLocaleString() : "N/A" }}</td>

          <td v-if="editingItem === item.id"><input v-model="updatedItem.restock_quantity" type="number" /></td>
          <td v-else>{{ item.restock_quantity }}</td>

          <td v-if="editingItem === item.id"><input v-model="updatedItem.location" type="text" /></td>
    ...
    <input v-model="newItem.restock_date" type="datetime-local" placeholder="Restock Date" />
